...
export ZEPPELIN_LOG_DIR=/opt/big-data/zeppelin-0.10.1/logs                      # Where log files are stored.  PWD by default.
# export ZEPPELIN_PID_DIR                       # The pid files are stored. ${ZEPPELIN_HOME}/run by default.
# export ZEPPELIN_WAR_TEMPDIR                   # The location of jetty temporary directory.
export ZEPPELIN_NOTEBOOK_DIR=/opt/notebook                  # Where notebook saved
...
##
export SPARK_HOME=/opt/big-data/spark-3.3.0                             # (required) When it is defined, load it instead of Zeppelin embedded Spark libraries
# export SPARK_SUBMIT_OPTIONS                   # (optional) extra options to pass to spark submit. eg) "--driver-memory 512M --executor-memory 1G".
# export SPARK_APP_NAME                         # (optional) The name of spark application.
export SPARK_CONF_DIR=/opt/big-data/spark-3.3.0/conf                         # (optional) In the zeppelin interpreter on docker mode, Need to set the local spark conf folder path
...
export HADOOP_CONF_DIR=/opt/big-data/hadoop-3.3.4/etc/hadoop                        # yarn-site.xml is located in configuration directory in HADOOP_CONF_DIR.
...
export HBASE_HOME=/opt/big-data/hbase-2.4.14                    # (require) Under which HBase scripts and configuration should be
export HBASE_CONF_DIR=/opt/big-data/hbase-2.4.14/conf                # (optional) Alternatively, configuration directory can be set to point to the directory that has hbase-site.xml
